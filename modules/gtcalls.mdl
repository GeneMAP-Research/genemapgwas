...
def check_bam_alignment() {
  if( params.bam_alignment == "" ) {
    error: println "ERROR: Please provide a value for 'bam_alignment' in the 'nextflow.config' file"
  }
}

def get_manisfest_bpm() {
  if( params.manifest_bpm == "" ) {
    error: println "ERROR: Please provide a value for 'manifest_bpm' in the 'nextflow.config' file"
  }
  else {
...
def get_manisfest_csv() {
  if( params.manifest_csv == "" ) {
    error: println "ERROR: Please provide a value for 'manifest_csv' in the 'nextflow.config' file"
  }
  else {
...
def get_cluster_file() {
  if ( params.cluster_file == "" ) {
    error: println "ERROR: Please provide a value for 'cluster_file' in the 'nextflow.config' file"
  }
  else {
...
def get_intensities() {
  if( params.idat_dir == "" ) {
    error: println "\nERROR: Oops! forgot to provide a value for 'idat_dir' in the 'nextflow.config' file?\n"
  }
  else {
    return channel
