...
if [ $# -lt 1 ]; then
    ...
    echo '##INFO=<ID=MR,Number=.,Type=String,Description="SNP Missing Rate">' >> ${emm}.annvar.vcf
    ...
    echo '##INFO=<ID=pBH,Number=.,Type=String,Description="Benjamin-Hochberg-adjusted P-value of association test">' >> ${emm}.annvar.vcf
    echo -e '#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO' >> ${emm}.annvar.vcf

    zcat $emm | \
        awk '$13 <= 5e-04' | \
        awk '{print "chr"$1"\t"$2"\t"$3"\t"$4"\t"$5"\t"".""\t"".""\t""AF="$7";""MR="$8";"";""N="$14";""BETA="$9";""SE="$10";""P="$13";""pBH="$15}' | \
        sed 's/chr//2' | \
        sed 's/chr23/chrX/1' | \
        sed 's/chr24/chrY/1' | \
        sed 's/chr25/chrMT/1' | 
