...
mishet=data.frame(FID=het$FID, IID=het$IID, het.rate=(het$OBS_CT - het$O.HOM.)/het$OBS_CT, mis.rate=mis$F_MISS)
...
png(filename = oput, width = 500, height = 480, units = "px", pointsize = 14,
    bg = "white",  res = NA)
par(mfrow=c(1,1))
plot(mishet$het.rate, 
     mishet$mis.rate, 
     xlab = "Heterozygous rate", 
     ylab = "Proportion of missing genotype", 
     main="Individual Missingness", pch=20,
     col=ifelse(mishet$het.rate < hetlower, "red", ifelse(mishet$het.rate > hetupper, "red", ifelse(mishet$mis.rate > 0.1, "red", "black"))))
